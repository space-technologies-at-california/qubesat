...
def runExperiment():
	...
	instructionArray = []
	for i in np.arange(config['START_LENGTH'], config['END_LENGTH'] + config['STEP_LENGTH'], config['STEP_LENGTH']):
		sequenceArgs = [i*ns, config['t_AOM'] * ns, config['t_readoutDelay'] * ns]
	...
	#Configure the DAQ
	config['task'], channel = DAQ.configureDAQ(RABI.Nsamples)
	...
	for i in range(NThrow):
		DAQ.readDAQ(config['task'], channel, RABI.Nsamples * 2, 10)
	...
	for n in np.arange(config['Navg']):
		...
		for t in np.arange(config['START_LENGTH'], config['END_LENGTH'] + config['STEP_LENGTH'], config['STEP_LENGTH']):

			#Read from the DAQ
			output = DAQ.readDAQ(config['task'], channel, RABI.Nsamples * 2, 10)
