...
def runExperiment():
	try:
		...
		SRSctl.setupSRSmodulation(my_SRS, 'T1seq') 

		instructionArray = []
		for i in np.arange(config['start_t'], config['end_t'] + config['step_t'], config['step_t']):
		...
		for n in np.arange(config['Navg']):
			...
			for f in np.arange(config['start_t'], config['end_t'] + config['step_t'], config['step_t']):
			    #Read from the DAQ
