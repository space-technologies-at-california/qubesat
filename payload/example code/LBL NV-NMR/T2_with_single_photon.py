...
def runExperiment():
	try:
		...
		for i in np.arange(config['start_tau'], config['end_tau'] + config['step_tau'], config['step_tau']):
			sequenceArgs = [i*ns, config['t_AOM'] * ns, config['t_readoutDelay'] * ns, config['t_pi'] * ns, config['IQpadding'], config['numberOfPiPulses']]
		...
		for n in np.arange(config['Navg']):
			...
			for f in np.arange(config['start_tau'], config['end_tau'] + config['step_tau'], config['step_tau']):
			    #Read from the DAQ
			    output = DAQ.readDAQ(config['task'], channel, T2.Nsamples * 2, config['DAQtimeout'])
